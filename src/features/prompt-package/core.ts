...
2. Пройди по каждому формату банка и перепиши легаси на современный стандарт: используй дефолтные сниппеты каталога (regex-snippets.toml) вместо легаси-паттернов, замени легаси-формы колонок (date#dd.MM.yyyy → date#dMy). Дефолтные сниппеты вставляй как есть, не подрезай под единственный пример. Информационные форматы (пустые COLUMNS) не переводи на транзакционные правила — у них свой стиль.

3. Убери избыточность. Форматы с одинаковым набором COLUMNS, описывающие один и тот же шаблон SMS (отличия только в префиксе/хвосте), объединяй в один: все примеры удаляемого файла до одного переноси в остающийся — ни один пример не должен пропасть из банка, — лишний файл удаляй блоком <delete> с причиной. Форматы с разным набором сущностей не сливай — разводи негативным lookahead, как в cookbook §5.
