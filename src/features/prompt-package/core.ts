...
export const PROMPT_PRESETS: Array<{ key: PromptPresetKey; task: string }> = [
  {
    key: "fixChanged",
    task: "Почини файлы, изменённые или добавленные в этом pull request (слои `pr` и `draft`): устрани из блока `intersections` всё, что относится к этим файлам, и сделай их совместимыми с остальными форматами банка. Новый формат из PR — сначала кандидат на вливание в существующий, а не готовый отдельный файл: прежде чем оставить его отдельным, примерь его примеры к форматам слоя `main` (существующие файлы обычно несут числовой id перед `.txt`, вида `perevod_123456.txt`). Кандидатов ищи по совпадающему набору COLUMNS и по общим статичным кускам текста в EXAMPLE; совпадение — повод для проверки, а не автоматика: заново выдели сущности из EXAMPLE обоих файлов и объединяй только при совпадении истинных наборов. Из нескольких кандидатов бери тот, чей regex и статичный текст ближе к новым примерам, — не по номеру в имени и не по свежести. Сливай в существующий файл: перенеси в него все примеры донора до одного, адаптируй его regex — минимальной правкой или переписыванием, на твой выбор, — а донора оформи блоком `<delete>` с причиной; после правки все прежние примеры файла обязаны матчиться по-прежнему. Отдельный новый файл оставляй только когда слияние потребовало бы существенно усложнить regex — назови причину в прозе. Форматы с действительно разным набором сущностей не сливай. Файлы, не тронутые в этом PR, не меняй — кроме слияния по правилу выше и случая, когда без правки соседа пересечение не устранить; каждый такой случай объясни в прозе. Место, которое правишь, приводи к дефолтным сниппетам каталога (у каждой сущности дефолт один; альтернатива — только по её триггеру, причину назови в отчёте); незатронутые места паттерна не переписывай. Якоря бери только из реальных текстов EXAMPLE. Перед выдачей проверь: каждый пример матчится своим regex, ни один пример не матчится regex другого формата банка, число capture-групп равно числу колонок.",
  },
  {
    key: "tidyBank",
